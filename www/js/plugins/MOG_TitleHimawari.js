...
Moghunter.himawari_char_number = Number(
  Moghunter.parameters["Char Number"] || 4
);
...
Moghunter.himawari_flower_x = Number(
  Moghunter.parameters["Flower X-Axis"] || 0
);
Moghunter.himawari_flower_y = Number(
  Moghunter.parameters["Flower Y-Axis"] || 0
);
Moghunter.himawari_rotation = String(
  Moghunter.parameters["Rotation"] || "true"
);
Moghunter.himawari_rotation_duration = Number(
  Moghunter.parameters["Rotation Time"] || 300
);
...
Scene_Title.prototype.initialize = function () {
  ...
  this._scenarioField.scale.x = 1.0;
  ...
  this._rt = [0, 0, 0];
  this._zt = [1.0, 0, 0];
  this._char_number = Math.min(
    Math.max(Moghunter.himawari_char_number, 1),
    999
  );
  this._char_number += 1;
  this._rotation = false;
  this._rotation_time = Math.min(
    Math.max(Moghunter.himawari_rotation_duration, 60),
    999
  );
  if (String(Moghunter.himawari_rotation) === "true") {
    this._rotation = true;
  }
...
Scene_Title.prototype.loadImages = function () {
  this._back_img = [
    ImageManager.loadTitle1("Background_1"),
...
// * Create Background
//==============================
var _alias_mog_himawari_createBackground =
...
Scene_Title.prototype.firstRefresh = function () {
  ...
  this._background[1].y =
    Graphics.boxHeight / 2 + (this._back_img[1].height * 18) / 100;
};
...
Scene_Title.prototype.update_rotation = function (sprite) {
  if (this._rt[0] === this._scenarioField.rotation) {
    this._rt[1] += 1;
  }
  ...
  this._scenarioField.rotation = this.sprite_move_to(
    this._scenarioField.rotation,
    this._rt[0],
    this.rotationSpeed()
  );
  if (this._zt[0] === this._scenarioField.scale.x) {
    this._zt[1] += 1;
  }
  ...
  this._scenarioField.scale.x = this.sprite_move_to(
    this._scenarioField.scale.x,
    this._zt[0],
    this.rotationSpeed()
  );
  this._scenarioField.scale.y = this._scenarioField.scale.x;
...
Scene_Title.prototype.refreshCharacters = function (sprite) {
  ...
  this._char_index += 1;
  if (this._char_index >= this._char_img.length) {
    this._char_index = 0;
  }
};
...
Scene_Title.prototype.setOrder = function (sprite, index, type) {
  ...
  for (var i = 0; i < this._duration[type].length; i++) {
    if (d < this._duration[type][i]) {
      d = this._duration[type][i];
    }
  }
...
Scene_Title.prototype.setInitialEffect = function (sprite, index, type) {
  ...
  if (this._side[type] === 0) {
    ...
    sprite.scale.x = -1.3;
    if (type === 0) {
      sprite.rotation -= Math.random() * 0.7;
    }
    this._side[type] = 1;
  } else {
    sprite.x = -(Graphics.boxWidth / 2);
...
Scene_Title.prototype.updateSprites = function (sprite, index, type) {
  ...
  if (this.needRefreshSprite(type, index)) {
    this.refreshSprite(sprite, index, type);
  }
};
...
Scene_Title.prototype.needRefreshSprite = function (type, index) {
  if (this._duration[type][index] <= 0) {
    return true;
  }
  return false;
};
...
Scene_Title.prototype.update_himawari = function () {
  if (this._firstRefresh && this._char_img[0].isReady()) {
    this.firstRefresh();
  }
