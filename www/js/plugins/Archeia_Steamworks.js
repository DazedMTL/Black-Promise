...
(function ($) {
  ...
  Archeia.Steamworks.DataManager_isDatabaseLoaded =
    DataManager.isDatabaseLoaded;
  DataManager.isDatabaseLoaded = function () {
    if (!Archeia.Steamworks.DataManager_isDatabaseLoaded.call(this))
      return false;
  ...
  DataManager.processSteamworksItemNotetags = function (group) {
    var note1 =
      /<(?:ACHIEVEMENT GRANT|achievement unlocked|achieved):[ ](.*),([+-]\d+)\/>/i;
    var note2 =
      /<(?:ACHIEVEMENT CLEAR|achievement taken|achievement rekted|unachieved):[ ](.*),([-+]\d+)\/>/i;
  ...
  DataManager.processSteamworksClassNotetags = function (group) {
    var note1 =
      /<(?:ACHIEVEMENT GRANT|achievement unlocked|achieved):[ ](.*),(LEARN|UNLEARN),(\d+)\/>/i;
    var note2 =
      /<(?:ACHIEVEMENT CLEAR|achievement taken|achievement rekted|unachieved):[ ](.*),(LEARN|UNLEARN),(\d+)\/>/i;
    for (var n = 1; n < group.length; n++) {
      var obj = group[n];
      var notedata = obj.note.split(/[\r\n]+/);
      ...
      for (var i = 0; i < notedata.length; i++) {
        var line = notedata[i];
        if (line.match(note1)) {
          obj.steamworks.achievementGrants[RegExp.$1] =
            obj.steamworks.achievementGrants[RegExp.$1] || {};
          obj.steamworks.achievementGrants[RegExp.$1].learn =
            obj.steamworks.achievementGrants[RegExp.$1].learn || [];
          obj.steamworks.achievementGrants[RegExp.$1].unlearn =
            obj.steamworks.achievementGrants[RegExp.$1].unlearn || [];
          var type = RegExp.$2.toLowerCase();
          switch (type) {
            case "learn":
              obj.steamworks.achievementGrants[RegExp.$1].learn.push(
                parseInt(RegExp.$3)
              );
              break;
            case "unlearn":
              obj.steamworks.achievementGrants[RegExp.$1].unlearn.push(
                parseInt(RegExp.$3)
              );
              break;
          }
        } else if (line.match(note2)) {
          obj.steamworks.achievementClears[RegExp.$1] =
            obj.steamworks.achievementClears[RegExp.$1] || {};
          obj.steamworks.achievementClears[RegExp.$1].learn =
            obj.steamworks.achievementClears[RegExp.$1].learn || [];
          obj.steamworks.achievementClears[RegExp.$1].unlearn =
            obj.steamworks.achievementClears[RegExp.$1].unlearn || [];
          var type = RegExp.$2.toLowerCase();
          switch (type) {
            case "learn":
              obj.steamworks.achievementClears[RegExp.$1].learn.push(
                parseInt(RegExp.$3)
              );
              break;
            case "unlearn":
              obj.steamworks.achievementClears[RegExp.$1].unlearn.push(
                parseInt(RegExp.$3)
              );
              break;
          }
  ...
  Game_System.prototype.getSteamUserObject = function () {
    if (SceneManager.steamworksInitialized()) {
      ...
      if ($.Param.debugMode)
        console.warn("[ARCHEIA STEAMWORKS] Steamworks is not Initialized.");
      return null;
    }
  };
  ...
  Game_System.prototype.getSteamName = function () {
    if (this.getCachedSteamUser()) {
      ...
      if ($.Param.debugMode)
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to obtain Steam Name. Is Steamworks Initialized?"
        );
      return null;
    }
  };
  ...
  Game_System.prototype.getSteamLevel = function () {
    if (this.getCachedSteamUser()) {
      ...
      if ($.Param.debugMode)
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to obtain Steam Level. Is Steamworks Initialized?"
        );
      return null;
    }
  };
  ...
  Game_System.prototype.getSteamAccountId = function () {
    if (this.getCachedSteamUser()) {
      ...
      if ($.Param.debugMode)
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to obtain Account Id. Is Steamworks Initialized?"
        );
      return null;
    }
  };
  ...
  Game_System.prototype.getSteamStaticAccountId = function () {
    if (this.getCachedSteamUser()) {
      ...
      if ($.Param.debugMode)
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to obtain Static Account Id. Is Steamworks Initialized?"
        );
      return null;
    }
  };
  ...
  Game_System.prototype.unlockAchievement = function (
    ...
    if (SceneManager.steamworksInitialized()) {
      steamworks.activateAchievement(
        ...
        function (err) {
          if (errorcallback) {
            ...
            if ($.Param.debugMode) {
              console.error(
                Archeia.Utils.sformat(
                  "[ARCHEIA STEAMWORKS] Error when trying to unlock achievement '{0}' OUTPUT: '{1}'",
                  achievement,
                  err
                )
              );
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to grant achievement. Is Steamworks Initialized?"
        );
      }
    }
  };
  ...
  Game_System.prototype.getAchievements = function () {
    if (SceneManager.steamworksInitialized()) {
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to retrieve achievements. Is Steamworks Initialized?"
        );
      }
      return null;
    }
  ...
  Game_System.prototype.getNumberOfAchievements = function () {
    if (SceneManager.steamworksInitialized()) {
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to retrieve achievement count. Is Steamworks Initialized?"
        );
      }
      return null;
    }
  ...
  Game_System.prototype.clearAchievement = function (
    ...
    if (SceneManager.steamworksInitialized()) {
        ...
        function (err) {
          if (errorcallback) {
            ...
            if ($.Param.debugMode) {
              console.error(
                Archeia.Utils.sformat(
                  "[ARCHEIA STEAMWORKS] Error when trying to clear achievement '{0}' OUTPUT: '{1}'",
                  achievement,
                  err
                )
              );
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to clear achievement. Is Steamworks Initialized?"
        );
      }
    }
  };
  ...
  Game_System.prototype.getAchievement = function (
    ...
    if (SceneManager.steamworksInitialized()) {
        ...
        function (err) {
          if (errorcallback) {
            ...
            if ($.Param.debugMode) {
              console.error(
                Archeia.Utils.sformat(
                  "[ARCHEIA STEAMWORKS] Error when trying to get achievement '{0}' OUTPUT: '{1}'",
                  achievement,
                  err
                )
              );
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to get achievement. Is Steamworks Initialized?"
        );
      }
    }
  };
  ...
  Game_System.prototype.getCurrentGameLanguage = function () {
    if (SceneManager.steamworksInitialized()) {
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to get game language. Is Steamworks Initialized?"
        );
      }
      return null;
    }
  ...
  Game_System.prototype.getNumberOfPlayers = function (
    ...
    if (SceneManager.steamworksInitialized()) {
        ...
        function (err) {
          if (errorcallback) {
            ...
            if ($.Param.debugMode) {
              console.error(
                Archeia.Utils.sformat(
                  "[ARCHEIA STEAMWORKS] Error when trying to get number of players. OUTPUT: '{0}'",
                  err
                )
              );
            }
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to get number of players. Is Steamworks Initialized?"
        );
      }
      return null;
    }
  ...
  Game_System.prototype.activateGameOverlay = function (option) {
    if (SceneManager.steamworksInitialized()) {
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to open steam overlay. Is Steamworks Initialized? Does the player have game overlay enabled?"
        );
      }
    }
  };
  ...
  Game_System.prototype.isGameOverlayEnabled = function () {
    if (SceneManager.steamworksInitialized()) {
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to get if steam overlay is enabled. Is Steamworks Initialized?"
        );
      }
      return null;
    }
  ...
  Game_System.prototype.activateGameOverlayToWebpage = function (url) {
    if (SceneManager.steamworksInitialized()) {
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to get open game overlay to specified url. Is Steamworks Initialized? Does the player have Game overlay Enabled?"
        );
      }
      return null;
    }
  ...
  Game_System.prototype.saveTextToCloud = function (
    file,
    contents,
    callback,
    errorcallback
    ...
    if (SceneManager.steamworksInitialized()) {
        ...
        function (err) {
          if (errorcallback) {
            ...
            if ($.Param.debugMode) {
              console.error(
                Archeia.Utils.sformat(
                  "[ARCHEIA STEAMWORKS] Error when trying to save file to cloud. OUTPUT: '{0}'",
                  err
                )
              );
            }
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to get save file to cloud. Is Steamworks Initialized? Does the player have cloud sync enabled?"
        );
      }
    }
  };

  Game_System.prototype.readTextFromCloud = function (
    file,
    callback,
    errorcallback
  ) {
    if (SceneManager.steamworksInitialized()) {
      steamworks.readTextFromFile(
        file,
        function (contents) {
          if (callback) {
            callback(contents);
        ...
        function (err) {
          if (errorcallback) {
            ...
            if ($.Param.debugMode) {
              console.error(
                Archeia.Utils.sformat(
                  "[ARCHEIA STEAMWORKS] Error when trying to read file from cloud. OUTPUT: '{0}'",
                  err
                )
              );
            }
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to get red file from cloud. Is Steamworks Initialized? Does the player have cloud sync enabled?"
        );
      }
    }
  };
  ...
  Game_System.prototype.isCloudSyncEnabledForUser = function () {
    if (SceneManager.steamworksInitialized()) {
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to get if cloud sync is enabled for user. Is Steamworks Initialized?"
        );
      }
      return null;
    }
  ...
  Game_System.prototype.isCloudEnabled = function () {
    if (SceneManager.steamworksInitialized()) {
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to get if cloud sync is enabled. Is Steamworks Initialized?"
        );
      }
      return null;
    }
  ...
  Game_System.prototype.enableCloud = function (flag) {
    if (SceneManager.steamworksInitialized()) {
      ...
      if ($.Param.debugMode) {
        console.warn(
          "[ARCHEIA STEAMWORKS] Unable to enable/disable cloud sync. Is Steamworks Initialized?"
        );
      }
    }
  };
  ...
  Game_System.prototype.getCloudQuota = function (callback, errorcallback) {
    if (SceneManager.steamworksInitialized()) {
      steamworks.getCloudQuota(
        function (total, available) {
          if (callback) {
        ...
        function (err) {
          if (errorcallback) {
            ...
            if ($.Param.debugMode) {
              console.error(
                Archeia.Utils.sformat(
                  "[ARCHEIA STEAMWORKS] Error when trying to get cloud quota. OUTPUT: '{0}'",
                  err
                )
              );
            }
  ...
  Game_System.prototype.onAchievementUnlocked = function (achievement) {
    $gameParty.members().forEach(function (member) {
      if (member.currentClass().steamworks.achievementGrants[achievement]) {
        if (
          member.currentClass().steamworks.achievementGrants[achievement].learn
        ) {
          for (
            var l = 0;
            l <
            member.currentClass().steamworks.achievementGrants[achievement]
              .learn.length;
            l++
          ) {
            member.learnSkill(
  ...
  Game_System.prototype.onAchievementCleared = function (achievement) {
    $gameParty.members().forEach(function (member) {
      if (member.currentClass().steamworks.achievementClears[achievement]) {
        if (
          member.currentClass().steamworks.achievementClears[achievement].learn
        ) {
          for (
            var l = 0;
            l <
            member.currentClass().steamworks.achievementClears[achievement]
              .learn.length;
            l++
          ) {
            member.learnSkill(
  ...
  Archeia.Steamworks.GameInterpreter_pluginCommand =
    ...
    if (command.toLowerCase() == "steamworks") {
      ...
      switch (action) {
        ...
        case "playercount":
          ...
          $gameSystem.getNumberOfPlayers(
            ...
            function (err) {
              if ($.Param.debugMode) {
                console.error(
                  "[ARCHEIA STEAMWORKS] An error occurred when attempting to get player count. Is Steamworks Initialized?"
                );
              }
        ...
        case "grantachievement":
          ...
          args = args.splice(1, args.length - 1);
          $gameSystem.unlockAchievement(
            args.join(""),
            $gameSystem.onAchievementUnlocked,
            null
          );
          break;
        ...
        case "getrekted": // I, Liquid, am a child at heart, this must be kept in in order for the children of the world to get their daily dose of memes.
          if (args.length < 2) return; // no achievement name given.
          args = args.splice(1, args.length - 1);
          $gameSystem.clearAchievement(
            args.join(""),
            $gameSystem.onAchievementCleared,
            null
          );
          break;
      ...
      Archeia.Steamworks.GameInterpreter_pluginCommand.call(
        this,
        command,
        args
      );
