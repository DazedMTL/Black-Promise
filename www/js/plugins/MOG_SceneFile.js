 ...
 *
 * 改造：剣崎宗二
 * layout.png→layout0.pngとすること。
 * layout1.png... 等（layout0を一ページとして含めたうえで）ページ数だけ用意する事。
 * （セーブデータが3ページならlayout0, layout1, layout2を用意）
 */
...
Moghunter.scFile_NumPages = Number(Moghunter.parameters['Save Pages'] || 5);
Moghunter.scFile_SPerPage = Number(Moghunter.parameters['Save Per Page'] || 3);
...
DataManager.maxSavefiles = function () {
	return Moghunter.scFile_NumPages * Moghunter.scFile_SPerPage;
};
...
Scene_File.prototype.create = function () {
	...
	if (this._listWindow._index < 0 || this._listWindow._index > (Moghunter.scFile_SPerPage * Moghunter.scFile_NumPages) - 1) { this._listWindow._index = 0 };
	this._Cursor.x = Moghunter.scFile_CursorX + (Moghunter.scFile_CursorS * (this._listWindow._index % Moghunter.scFile_SPerPage));
};
...
Scene_File.prototype.createLayout = function () {
	this._layout = new Sprite(ImageManager.loadMenusFile("layout0"));
...
// * update Layout
//==============================
Scene_File.prototype.updateLayout = function () {
	var index = this._listWindow ? this._listWindow._index : 0;
	var page = Math.floor(index / Moghunter.scFile_SPerPage);
	if (page != this._currentPage) {
		this._layout.bitmap = ImageManager.loadMenusFile("layout" + page);
...
Scene_File.prototype.updateCursor = function () {
	var nx = Moghunter.scFile_CursorX + (Moghunter.scFile_CursorS * (this._listWindow._index % Moghunter.scFile_SPerPage));
	var ny = Moghunter.scFile_CursorY;
	this._Cursor.x = this.barCMT(this._Cursor.x, nx);
	this._Cursor.y = this.barCMT(this._Cursor.y, ny);
	if (this._Cursor.opacity < 120) { this._Cursor.opacity += 5 };
};
...
Scene_File.prototype.nextIndex = function (value) {
	...
	var maxIndex = (Moghunter.scFile_SPerPage * Moghunter.scFile_NumPages) - 1;
	if (this._listWindow._index < 0) { this._listWindow._index = maxIndex };
	if (this._listWindow._index > maxIndex) { this._listWindow._index = 0 };
};
...
Scene_File.prototype.checkTouchFile = function () {
	...
	var onfile = false
	for (i = 0; i < Moghunter.scFile_SPerPage; i++) {
		if (this.isOnFile(i)) {
			this._listWindow._index = i + this._currentPage * Moghunter.scFile_SPerPage;
			onfile = true;
		};
	};
	if (!onfile) { return };
...
Scene_File.prototype.barCMT = function (value, real_value) {
	...
	var dnspeed = 5 + (Math.abs(value - real_value) / 5);
	if (value > real_value) {
		value -= dnspeed;
