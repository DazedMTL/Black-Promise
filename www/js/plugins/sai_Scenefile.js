...
(function () {
  ...
  sai.ss_percent = Number(parameters["SS Percent"] || 0.3);
  ...
  Scene_File.prototype.create = function () {
    ...
    //ステータスウインドウの配置
    this._statusWindow = new Window_SavefileStatus(
      Number(sai.state_x),
      Number(sai.state_y),
      Number(sai.state_width),
      Number(sai.state_height)
    );
  ...
  Window_SavefileList.prototype.initialize = function (x, y, width, height) {
    Window_Selectable.prototype.initialize.call(
      ...
      Number(sai.list_x),
      Number(sai.list_y),
      Number(sai.list_width),
      Number(sai.list_height)
    );
    this.activate();
    this._mode = null;
  };
  ...
  Window_SavefileStatus.prototype.drawFileId = function (id, x, y) {
    this.drawTextEx(
      sai.location_text + " ",
      Number(sai.locationtext_x),
      Number(sai.locationtext_y),
      120
    );
    this.drawTextEx(
      sai.gold_text + " ",
      Number(sai.goldtext_x),
      Number(sai.goldtext_y),
      120
    );
  };
  ...
  Window_SavefileStatus.prototype.drawContents = function (info, x, valid) {
    this.drawText(
      info.savetime,
      Number(sai.savetime_x),
      Number(sai.savetime_y),
      200
    );
    if (info.location) {
    ...
    if (info.level) {
      this.drawText(
        TextManager.levelA + " " + info.level,
        Number(sai.level_x),
        Number(sai.level_y),
        60
      );
    }
    if (typeof info.gold === "number") {
      //値が数字の時に通る
      this.drawText(
    ...
    if (valid) {
      this.drawPartyCharacters(
        info,
        Number(sai.members_x),
        Number(sai.members_y)
      );
    }
  };
