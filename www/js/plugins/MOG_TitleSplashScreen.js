...
Moghunter.title_splash_number = Number(
  Moghunter.parameters["Number of Logos"] || 1
);
Moghunter.title_splash_duration = Number(
  Moghunter.parameters["Slash Duration"] || 60
);
Moghunter.title_splash_fade_speed = Number(
  Moghunter.parameters["Splash Fade Duration"] || 2
);
Moghunter.title_full_screen_mode =
  Moghunter.parameters["Full Screen Mode"] || false;
...
Scene_Boot.prototype.start = function () {
  if (Moghunter.title_full_screen_mode == "true") {
...
Scene_Splash_Screen.prototype.create = function () {
  ...
  this._splash_data = [
    0,
    0,
    Math.max(Moghunter.title_splash_duration, 1),
    Math.max(Moghunter.title_splash_fade_speed, 1),
  ];
  this._splash_img = [];
...
Scene_Splash_Screen.prototype.update = function () {
  ...
  if (this._splash_data[1] <= 0) {
    ...
    if (
      ...
      this._splash_sprite.opacity > 60
    ) {
