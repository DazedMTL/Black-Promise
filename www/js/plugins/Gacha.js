...
(function () {
  ...
  var message = String(
    parameters["Help Message Text"] || "1回Required Amount\\Gでガチャを引きます"
  );
  ...
  Scene_Gacha.prototype.createCommandWindow = function () {
    this._commandWindow = new Window_GachaCommand(
      this._goldWindow.x,
      this._purchaseOnly
    );
  ...
  Scene_Gacha.prototype.createGetCommandWindow = function () {
    this._getCommandWindow = new Window_GachaGetCommand(
      Graphics.boxWidth,
      false
    );
    this._getCommandWindow.y =
      Graphics.boxHeight - this._getCommandWindow.height;
          ...
          if (!!this._item.meta.gachaImage) {
            ...
            if (rank > 0 && rank < 6) {
              if (rankEffect[rank - 1] > 0) {
                ...
                this._rankSprite.x =
                  this._getWindow.width / 2 + this._getWindow.x - 20;
                this._rankSprite.y =
                  this._getWindow.contentsHeight() + this._getWindow.y;
                if (itemDescEnable) {
  ...
  Window_GachaCommand.prototype.makeCommandList = function () {
    this.addCommand(
      buttonText,
      "gacha",
      $gameParty.gold() >= amount && !!this._lotIsValid
    );
  ...
  Window_GachaGetCommand.prototype = Object.create(
  ...
  Window_GachaGet.prototype.refresh = function () {
    ...
    if (!item || !item.meta.gachaImage) {
      ...
      var bitmap;
      bitmap = ImageManager.loadBitmap(
        "img/gacha/",
        this._item.meta.gachaImage
      );
  ...
  Sprite_GachaEffect.prototype.startAnimation = function (
    animation,
    mirror,
    delay
  ) {
